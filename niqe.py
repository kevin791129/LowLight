...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    # scipy removed imresize as of 1.3.0
    # img2 = scipy.misc.imresize(img, 0.5, interp='bicubic', mode='F')
    img2 = Image.fromarray(img, mode='F')
    img2 = np.array(img2.resize((img2.size[0] // 2, img2.size[1] // 2), Image.BILINEAR))
...
def niqe(inputImgData, param_path=None):

    patch_size = 96
    module_path = dirname(__file__)

    # TODO: memoize
    params = scipy.io.loadmat(param_path if param_path is not None else join(module_path, 'data', 'niqe_image_params.mat'))
